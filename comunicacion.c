...
void init_semaforos(struct globales *vglobales){
   sem_init(&vglobales->sem_progreso, 1, 1);
   sem_init(&vglobales->sem_instancia, 1, 1);
   sem_init(&vglobales->sem_solucionador, 1, 1);
}

void modificarProgreso(struct globales *vglobales){
    sem_wait(&vglobales->sem_progreso);
    vglobales->progreso += 1;
    sem_post(&vglobales->sem_progreso);
}
